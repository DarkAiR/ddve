...
class DeliveryConditionsViewDeliveryConditions extends JView
{
    function display($tpl = null)
    {
        ...
        JToolBarHelper::title( JText::_( 'Delivery Condition Manager' ), 'generic.png' );
        ...
        $query = 'SELECT COUNT(*) FROM #__delivery_condition AS cd '.$where;
        $db->setQuery( $query );
        $total = $db->loadResult();
        $pageNav = new JPagination( $total, $limitstart, $limit );

        // Get data from the model
        $query  = 'SELECT *  FROM #__delivery_condition AS cd ' . $where . $orderby;
