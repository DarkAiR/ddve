...
class ActionsListViewActionsList extends JView
{
    function display($tpl = null)
    {
        ...
        JToolBarHelper::title( JText::_( 'Actions Manager' ), 'generic.png' );
        ...
        $query = 'SELECT COUNT(*) FROM #__actions AS cd '.$where;
        $db->setQuery( $query );
        $total = $db->loadResult();
        $pageNav = new JPagination( $total, $limitstart, $limit );

        // Get data from the model
        $query  = 'SELECT *  FROM #__actions AS cd ' . $where . $orderby;
