...
else
{
    ...
    do
    {
        ...
        $cartProdId = $cart[$i]["product_id"];
        $cartProdQuantity = $cart[$i]["quantity"];
        
        $weight_total       = ($ps_product->get_weight( $cartProdId ) * $cartProdQuantity) . ' ' . $ps_product->get_field( $cartProdId, "product_weight_uom" );
        ...
        $price = $ps_product->get_adjusted_attribute_price( $cartProdId, $cart[$i]["description"] );
        $price["product_price"] = $GLOBALS['CURRENCY']->convert( $price["product_price"], $price["product_currency"] );
        $amount += $cartProdQuantity;
        $product_parent_id = $ps_product->get_field( $cartProdId, "product_parent_id" );
        if( @$auth["show_price_including_tax"] == 1 )
        {
            $my_taxrate = $ps_product->get_product_taxrate( $cartProdId );
            $price["product_price"] *= ( $my_taxrate + 1);
        }
        $subtotal = round( $price["product_price"], 2 ) * $cartProdQuantity;
        $total += $subtotal;
        
        $flypage_id = $product_parent_id;
        if( $flypage_id == 0 )
            $flypage_id = $cartProdId;
        ...
        {
            $url = $sess->url( URL . "index.php?page=shop.product_details&flypage=$flypage&category_id=$category_id&product_id=$cartProdId" );
        }
        $html = str_replace( "_", " ", $ps_product->getDescriptionWithTax( $_SESSION['cart'][$i]["description"], $cartProdId ) ) . " ";
        if( $product_parent_id )
        {
            $db_detail = $ps_product->attribute_sql( $cartProdId, $product_parent_id );
            while( $db_detail->next_record() )
                $html .= $db_detail->f( "attribute_value" ) . " ";
        }
        $minicart[$ci]['product_id']        = $cartProdId;
        $minicart[$ci]['url']               = $url;
        $minicart[$ci]['product_name']      = shopMakeHtmlSafe( $ps_product->get_field( $cartProdId, "product_name" ) );
        $minicart[$ci]['quantity']          = $cartProdQuantity;
        $minicart[$ci]['price']             = $CURRENCY_DISPLAY->getFullValue( $subtotal );
        $minicart[$ci]['attributes']        = $html;
        $minicart[$ci]['weight']            = $weight_total;
