...
while ($db->next_record())
{
	...
	{
		$bFirst = false;
		echo "<div class='module_vm_filter'>";
		echo '<fieldset>';
		echo '  <legend>'.$db->f('parameter_label').'</legend>';
	}
...
//http://ddve.ru/index.php?page = shop.browse&option = com_virtuemart&Itemid = 64&product_type_id = 1&product_type_1_has_fish_comp = find_in_set_any&product_type_1_has_fish[]=%D0%A3%D0%B3%D0%BE%D1%80%D1%8C
	echo '<form class="filter_form" action="'.
			$sess->url(
	...
	{
		echo '<input type="checkbox" name="'.$item_name.'[]" value="'.$field.'" ';
		echo ($selected_value[$field]==1)? 'checked="checked"' : '';
		echo '> '.$field.' ';
	}
	echo '    <input type="submit" class="submit" value="Выбрать">';
	echo '</form>';
}
if( $bFirst == false )
{
